...
void GFile::ToFile(char *fileName)
{
    isMem = false;
    CreateFile(fileName);
    handle = fopen(fileName, "r+");
...
void GFile::DumpData(char *fileName)
{
...
bool GFile::FileExists(char *fileName)
{
    FILE *file;
...
void GFile::CreateFile(char *fileName)
{
    FILE *tmp = fopen(fileName, "w");
    fclose(tmp);
}
